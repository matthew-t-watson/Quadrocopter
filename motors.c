...
void update_motors_single_shot()
{
	...
	else
	{
		...
		OC2_output = 0.5*PID_XOUTPUT + 0.5*PID_YOUTPUT - PID_ZOUTPUT + MOTOR_MIN + MOTOR_RANGE*throttle;
		OC3_output = -0.5*PID_XOUTPUT + 0.5*PID_YOUTPUT + PID_ZOUTPUT + MOTOR_MIN + MOTOR_RANGE*throttle;
		OC4_output = -0.5*PID_XOUTPUT + -0.5*PID_YOUTPUT - PID_ZOUTPUT + MOTOR_MIN + MOTOR_RANGE*throttle;
...
void update_PID()
{
	...
	if(XINTEGRAL > 1) {XINTEGRAL = 1;}
	else if(XINTEGRAL < -1) {XINTEGRAL = -1;}
	if(YINTEGRAL > 1) {YINTEGRAL = 1;}
	else if(YINTEGRAL < -1) {YINTEGRAL = -1;}
